...
export default function VehiclesCareSection() {
  ...
  return (
    <section className="relative w-full py-10 sm:py-14 bg-[#040406] text-white overflow-hidden border-t border-white/[0.04]">
        ...
        <div className="flex items-end justify-between gap-4 w-full">
          ...
          >
            ...
            <p className="font-body text-neutral-300/80 text-xs sm:text-[0.88rem] lg:text-[0.92rem] leading-relaxed max-w-xl mt-2">
              From everyday commuters to luxury automobiles, STYLEIN provides professional care and maintenance solutions for a wide range of vehicles.
            </p>
          </motion.div>
      ...
      {/* Real API Brands Horizontal Showcase Track */}
      <div className="max-w-[1140px] mx-auto px-6 sm:px-8 lg:px-10 w-full">
        <div
          ref={scrollRef}
          className="flex items-stretch gap-4 sm:gap-5 overflow-x-auto overflow-y-hidden pb-6 pt-1 snap-x snap-mandatory scroll-smooth [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden w-full"
        >
          {activeBrands && activeBrands.length > 0 ? (
            activeBrands.map((brand, idx) => (
              <AboutBrandCard key={brand._id || brand.id || idx} brand={brand} index={idx} />
            ))
          ) : (
            <div className="text-neutral-500 text-xs py-4">Loading supported marques...</div>
          )}
