...
import BrandsShowcaseSkeleton from './BrandsShowcaseSkeleton';
...
export default function FeaturedBrandsShowcase() {
  ...
  return (
    <section id="featured-brands" className="relative w-full py-6 sm:py-9 lg:py-10 px-6 sm:px-10 lg:px-12 bg-[#040406] overflow-hidden border-t border-white/[0.04]">
      ...
      <div className="relative z-10 max-w-[1160px] mx-auto flex flex-col items-center gap-5 sm:gap-7 w-full">
        ...
        <motion.div variants={gridVariants} initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.2 }} className="w-full grid grid-cols-1 lg:grid-cols-12 items-center gap-6 lg:gap-5 mt-1">
          ...
          <motion.div variants={colRight} className="order-3 lg:col-span-3 w-full hidden lg:flex items-center justify-end">
            <BrandShowcaseRight brand={currentBrand} activeIndex={activeIndex} totalBrands={total} onSelect={(idx) => setActiveIndex(idx)} onPrev={handlePrev} onNext={handleNext} />
          </motion.div>
        </motion.div>
      </div>
    </section>
