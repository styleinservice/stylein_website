...
export default function FAQAccordionItem({ item, index, isOpen, onToggle }) {
  ...
  return (
    <motion.div
      ...
      className={`rounded-xl sm:rounded-2xl border transition-all duration-500 ease-[cubic-bezier(0.22,1,0.36,1)] overflow-hidden text-left ${
        isOpen
    ...
    >
      {/* Accordion Trigger Header (Scaled to 80% compact size) */}
      <button
        type="button"
        onClick={onToggle}
      ...
      >
        <div className="flex items-center gap-3 sm:gap-4 flex-1">
          {/* Compact Index Counter Pill */}
          <div
            className={`w-7 h-7 rounded-lg flex items-center justify-center text-[0.68rem] font-bold tracking-tight font-heading shrink-0 transition-colors duration-500 ease-[cubic-bezier(0.22,1,0.36,1)] ${
              isOpen
          ...
          {/* Question Text */}
          <span
            className={`text-[0.88rem] sm:text-[0.95rem] font-semibold tracking-tight font-heading transition-colors duration-500 ease-[cubic-bezier(0.22,1,0.36,1)] ${
        ...
        {/* Compact Chevron Indicator */}
        <div
          className={`w-7 h-7 rounded-lg flex items-center justify-center border shrink-0 transition-all duration-500 ease-[cubic-bezier(0.22,1,0.36,1)] ${
            isOpen
      ...
      {/* Accordion Expandable Answer Body (Clean text without star emoji/icon) */}
        ...
        {isOpen && (
          ...
          >
            <motion.div
              ...
              className="px-4 pb-4.5 sm:px-5 sm:pb-5 pt-0 pl-[44px] sm:pl-[48px]"
            >
